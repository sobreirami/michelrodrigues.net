        ...
        <Nav>
          <List>
            {pages.map(page => {
              ...
              const isHovered = hovered === page.path

              return (
                <li key={page.path.toString()}>
...
const Animation = forwardRef((props, ref) => {
  return (
    <Box
      ref={ref}
      onMouseEnter={() => props.icon.current?.play()}
      onMouseLeave={() => props.icon.current?.stop()}
    >
      {props.children}
    </Box>
  )
